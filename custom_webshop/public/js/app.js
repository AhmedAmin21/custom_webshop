...
const INITIAL_PRODUCTS = window.INITIAL_PRODUCTS || [];
...
function initializeState() {
    ...
    state.cart = JSON.parse(localStorage.getItem("cnc_cart") || "[]");
...
function handleNavigation() {
    const view = window.CURRENT_ROUTE || "home";
    ...
    if (activeSection) {
        ...
        if (view === "home") { renderHomeView(); startSlideshowRotation(); } else {
            ...
            if (view === "catalog") { 
                ...
                renderCatalogView(categoryFilter, brandFilter); 
            }
            else if (view === "product") { 
                renderDetailView(window.INITIAL_PRODUCT?.id || urlParams.get("id")); 
            }
...
function renderHomeView() {
    ...
    Object.keys(CATEGORIES).forEach(key => {
        ...
        header.innerHTML = `
            <div style="display:flex; flex-direction:column; gap: 4px;">
                <div style="display:flex; align-items:center; gap: 12px;">
                    ...
                    <a href="/all-products?category=${key}" class="carousel-view-all">View All</a>
                </div>
        ...
        categoryProducts.forEach(prod => {
            ...
            card.innerHTML = `
                <div class="product-img-wrapper" onclick="window.location.href='/product?id=${prod.id}'">
                    <img src="${prod.image}" alt="${prod.name}" class="product-img">
                    ${badgeHTML}
                </div>
                <div class="product-info">
                    <span class="product-cat">${prod.categoryName}</span>
                    <h3 class="product-name" onclick="window.location.href='/product?id=${prod.id}'">${prod.name}</h3>
    ...
    if (brandGrid) {
        ...
        uniqueBrands.forEach(brand => {
            ...
            card.onclick = () => {
                window.location.href = `/all-products?brand=${encodeURIComponent(brand)}`;
            };
...
function renderDetailView(productId) {
    ...
    if (!prod && window.INITIAL_PRODUCT && window.INITIAL_PRODUCT.id === productId) {
        prod = window.INITIAL_PRODUCT;
    }
    ...
    if (relatedGrid) {
        ...
        items.forEach(item => {
            ...
            card.className = "product-card";
            card.innerHTML = `
                <div class="product-img-wrapper" onclick="window.location.href='/product?id=${item.id}'">
                    <img src="${item.image}" alt="${item.name}" class="product-img">
                </div>
                <div class="product-info">
                    <span class="product-cat">${item.categoryName}</span>
                    <h4 class="product-name" onclick="window.location.href='/product?id=${item.id}'">${item.name}</h4>
    ...
    if (state.cart.length === 0) {
        cartList.innerHTML = `
            <div class="drawer-empty-text" style="padding: 48px 0; text-align: center;">
                ...
                <a href="/all-products" class="btn btn-secondary btn-sm" style="margin-top: 16px;">Browse Parts Catalog</a>
            </div>
        `;
        updateCartTotals(0);
        return;
...
function initCheckoutForm() {
    ...
    form.addEventListener("submit", (e) => {
        e.preventDefault();
        if (state.cart.length === 0) {
            showToast("Your selection list is empty.", "error");
            return;
        }
        ...
        frappe.call({
            ...
            callback: function(r_addr) {
                ...
                // Update cart address
                const addr_name = r_addr.message;
                frappe.call({
                    method: "custom_webshop.shopping_cart.cart_override.update_cart_address",
                    args: {
                        address_type: "Shipping",
                        address_name: addr_name
                    },
                    callback: function(r_upd) {
                        // Place order
                        frappe.call({
                            method: "custom_webshop.shopping_cart.cart_override.place_order_from_cart",
                            callback: function(r_ord) {
                                submitBtn.innerHTML = originalText;
                                submitBtn.classList.remove("disabled");
                                if (!r_ord.exc && r_ord.message) {
                                    state.cart = [];
                                    saveCartToStorage();
                                    updateCartBadgeCount();
                                    window.location.href = "/payment?order_id=" + r_ord.message;
                                } else {
        ...
        slideDiv.innerHTML = `
            ...
            <div class="hero-slide-content">
                ...
                <div class="hero-actions">
                    ...
                    <a href="/all-products" class="btn btn-secondary">Browse All Catalog</a>
                </div>
            </div>
        `;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    handleNavigation(); // Called explicitly // Trigger initial routing match
