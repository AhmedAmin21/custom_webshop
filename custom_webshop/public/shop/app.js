...
function applyLanguage(lang) {
    ...
    if (typeof renderCatalogGrid === 'function' && document.getElementById("catalog-view")) { renderCatalogGrid(); }
    if (typeof renderCartList === 'function' && document.getElementById("cart-view")) { renderCartList(); updateCartTotals(); }
}
...
function initCurrentPage() {
    if (typeof redirectLegacyHash === "function" && redirectLegacyHash()) return;
    ...
    switch (page) {
        ...
        case "catalog": {
            ...
            renderCatalogView(params.get("category"), params.get("brand"));
            break;
        }
        case "detail":
            stopSlideshowRotation();
            renderDetailView(window.SHOP_BOOT?.item_code || "");
            break;
        case "cart":
            stopSlideshowRotation();
            renderCartView();
...
function renderHomeView() {
    ...
    Object.keys(CATEGORIES).forEach(key => {
        ...
        header.innerHTML = `
            <div style="display:flex; flex-direction:column; gap: 4px;">
                <div style="display:flex; align-items:center; gap: 12px;">
                    ...
                    <a href="/shop/catalog?category=${key}" class="carousel-view-all">View All</a>
                </div>
        ...
        categoryProducts.forEach(prod => {
            ...
            card.innerHTML = `
                <div class="product-img-wrapper" onclick="ShopNav.go(ShopNav.product('${prod.id}'))">
                    <img src="${prod.image}" alt="${prod.name}" class="product-img">
                    ${badgeHTML}
                </div>
                <div class="product-info">
                    <span class="product-cat">${prod.categoryName}</span>
                    <h3 class="product-name" onclick="ShopNav.go(ShopNav.product('${prod.id}'))">${prod.name}</h3>
    ...
    if (brandGrid) {
        ...
        uniqueBrands.forEach(brand => {
            ...
            card.onclick = () => {
                ShopNav.go(ShopNav.catalog({ brand: brand }));
            };
...
function getCatalogFilterParams() {
    const params = new URLSearchParams(window.location.search);
    const category =
        document.querySelector('input[name="category-filter"]:checked')?.value ||
        params.get("category") ||
        "all";
    const brand =
        document.querySelector('input[name="brand-filter"]:checked')?.value ||
        params.get("brand") ||
        "all";
    const query = {};
...
window.onCatalogFilterChange = function () {
    const next = ShopNav.catalog(getCatalogFilterParams());
...
function renderCatalogView(activeCategory = null, activeBrand = null) {
    ...
    Object.keys(CATEGORIES).forEach(key => {
        ...
        option.innerHTML = `
            <input type="radio" name="category-filter" value="${key}" ${isChecked ? 'checked' : ''} onchange="onCatalogFilterChange()">
    ...
    const uniqueBrands = ["all", ...new Set(state.products.map(p => p.brand).filter(Boolean))];
    ...
    uniqueBrands.forEach(brand => {
        ...
        option.innerHTML = `
            <input type="radio" name="brand-filter" value="${brand}" ${isChecked ? 'checked' : ''} onchange="onCatalogFilterChange()">
    ...
    if (state.cart.length === 0) {
        cartList.innerHTML = `
            <div class="drawer-empty-text" style="padding: 48px 0; text-align: center;">
                ...
                <a href="/shop/catalog" class="btn btn-secondary btn-sm" style="margin-top: 16px;">Browse Parts Catalog</a>
            </div>
        `;
        updateCartTotals(0);
        return;
    ...
    state.cart.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <div class="cart-item-info">
                <a href="${ShopNav.product(prod.id)}" class="cart-item-title">${prod.name}</a>
...
function renderAdminInventory() {
    ...
    state.products.forEach(prod => {
        ...
        tr.innerHTML = `
            ...
            <td style="color:var(--text-muted); font-size:0.85rem;">Read-only</td>
        `;
...
function initCheckoutForm() {
    ...
    form.addEventListener("submit", (e) => {
        ...
        form.reset();
        ShopNav.go(ShopNav.payment(checkoutPayload.id));
    });
}
...
// submitPaymentProof is implemented in erp-integration.js
...
function renderAdminSlideshowTable() {
    ...
    state.slides.forEach((slide, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td><code>${slide.link || '/shop/catalog'}</code></td>
            <td>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 3. Page render runs after ERP bootstrap (erp-integration.js)
    // initCurrentPage() is invoked from bootstrapShop once API data is ready.
    ...
    initMobileMenu();
    const page = window.SHOP_BOOT?.active_page || "home";
    if (page === "catalog") initViewToggle();
    if (page === "admin") {
        initAdminTabs();
